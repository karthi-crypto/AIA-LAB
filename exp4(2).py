...
def print_moves_with_scores(board):
    print("Possible AI moves (position : minimax score):")
    for i in range(9):
        if board[i] == ' ':
            board[i] = 'O'
            score = minimax(board, 0, False)
            board[i] = ' '
            print(f"Position {i + 1} : Score {score}")
    print()
...
def main():
    ...
    while True:
        ...
        print_moves_with_scores(board)  # Show AI moves and scores before AI moves
